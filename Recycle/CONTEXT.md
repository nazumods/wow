...
- **Manual marking:** hooks `HandleModifiedItemClick` (not a frame script). The chosen modifier (`settings.modKey`) + **right-click** on a bag item toggles `db.itemsToSell[itemID]`; the gate is `IsControlKeyDown`/`IsShiftKeyDown`/`IsAltKeyDown` matched against `GetMouseButtonClicked() == "RightButton"`. The wrapper **returns the stock function's boolean** (`did I handle this click`) — Blizzard callers branch on it (`if HandleModifiedItemClick(...) then return`), so swallowing it would make a handled click fall through to an unintended split/pickup/DressUp.
