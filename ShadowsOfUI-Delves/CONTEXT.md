...
- **Calibrate-in-game spots** (each has a safe fallback; `/sdelves` surfaces the live values):
  ...
  `readTier`) and falls back to the unknown bucket (still counted in the aggregate) — `/sdelves` now
  surfaces the structured `GetActiveDelveTier().tier`/`difficultyID` beside that widget scrape as the
  #411 probe comparing the two; the **POI
