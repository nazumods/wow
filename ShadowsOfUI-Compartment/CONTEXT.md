...
- **Position is stored scale-independently.** `savePosition` converts `GetLeft/GetTop` by `frame:GetEffectiveScale() / UIParent:GetEffectiveScale()`, then `applyPosition` re-anchors `TOPLEFT → UIParent BOTTOMLEFT`. This survives the minimap cluster having a non-1 scale. `db.position == nil` means "leave Blizzard's anchor".
