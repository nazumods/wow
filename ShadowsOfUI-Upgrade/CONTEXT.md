...
link) and **empty gem sockets** (from the data layer's stored per-slot count) on equipped gear, both
...
ShadowsOfUI_UpgradeApi:RecommendedGem(charName)        → EnchantSuggestion?
    -- which gem to socket (one per character — sockets take any gem).  ClassCodex's
    -- per-spec primary gem when installed, else the bundled secondary-stat gem for the
    -- top stat.  Resolve a name from `.name` / `.id` (an item; via GetItemInfo).
