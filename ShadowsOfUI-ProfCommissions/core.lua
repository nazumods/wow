...
-- Layout knobs shared by the feature files. They live on the namespace rather than as per-file
-- locals so /sprofcomm can retune them from here; there is no saved data, so every value is back at
-- its default next session.
...
SlashCmdList["SUI_PROFCOMM"] = function(msg)
  ...
  ns.Print(("Crafting-order hook %s; reward icon size %d, money reserve %d."):format(
    ...
    ns.tuning.iconSize, ns.tuning.moneyReserve))
end
