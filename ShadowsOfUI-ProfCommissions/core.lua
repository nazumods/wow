...
ns.tuning = {
  iconSize = 18,
  iconGap = 4, -- px between adjacent reward icons; matches the glow's spread so two adjacent
               -- rare rewards' haloes meet without overlapping
...
-- /sprofcomm size <n>    — retune the reward icon size live; takes effect on the next full populate,
--                          i.e. reopen the Crafting Orders window. A header re-sort is NOT enough:
--                          it reuses the cells already drawn, so the old values stay on screen.
--                          A tuning aid, since exact pixel sizing is easier eyeballed than guessed.
...
SlashCmdList["SUI_PROFCOMM"] = function(msg)
  ...
  if g then
    ...
    ns.Print(("Rare-reward glow threshold set to quality %d (reopen the orders window to apply)."):format(g))
    return
  end
  local n = tonumber(msg:match("^%s*size%s+(%d+)"))
  if n then
    ns.tuning.iconSize = n
    ns.Print(("Reward icon size set to %d (reopen the orders window to apply)."):format(n))
    return
  end
  local r = tonumber(msg:match("^%s*reserve%s+(%d+)"))
  if r then
    ns.tuning.moneyReserve = r
    ns.Print(("Money-zone reserve set to %d px (reopen the orders window to apply)."):format(r))
    return
  end
