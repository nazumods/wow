...
- **Guidance** — `updateWaypoint` sets a user waypoint to the first quest giver in `WaypointOrder[mapId]` with an incomplete quest, but only after `C_Map.CanSetUserWaypointOnMap(mapId)` says the map takes one (in practice Elwynn yes, DMF island no). A candidate whose `SetUserWaypoint` returns `false` is skipped and the next quest giver tried; the attempt is recorded in `lastWaypoint` for `/sdmf`. `refreshMapPins` draws clickable QuestGiver pins on the world map canvas for the same NPCs (pins are **pooled** via `acquirePin`/`clearMapPins` and reused across refreshes rather than re-`CreateFrame`d each time). Both refresh on `QUEST_ACCEPTED` / `QUEST_TURNED_IN` and on world-map show/`MapChanged`.
...
- **`/sdmf`** — dev helper: prints the mouseover/target NPC id and player map coords to populate `QuestGiverNpcs` / `NpcPositions`, then a second line with `inDMF`, the current map's `CanSetUserWaypointOnMap`, and the last waypoint attempt (`last=<label> wasSet=<bool|nil>`).
...
- **Waypoint support is queried, not assumed.** Darkmoon Island (map 407) doesn't accept a user waypoint, so it relies on map pins only — but that's read from `C_Map.CanSetUserWaypointOnMap` each time rather than hardcoded, since which maps allow waypoints is Blizzard's to change. `WaypointOrder` is a visit order, not a support allowlist: it lists 407 too.
- **`SetUserWaypoint`'s `wasSet` return is 12.1-only.** Added in PTR build 68824; clients before it return nothing. Test it as `wasSet ~= false`, never `if wasSet then`, or waypoints break on every pre-12.1 client.
