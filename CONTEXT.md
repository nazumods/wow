...
| **ShadowsOfUI-PostOffice** | Headless mailbox helper re-derived from Postal's pure-logic modules: **Rake** (reports coin collected per visit), **TradeBlock** (raises the `BlockTrades` CVar via `SetTemporaryCVar` while the mailbox is open), **Wire** (fills a blank `SendMailSubjectEditBox` with the coin amount via a lazy `SendMailMoney` value-changed hook). Mailbox lifecycle via `PLAYER_INTERACTION_MANAGER_FRAME_SHOW/HIDE` filtered to `MailInfo`. Per-feature Settings toggles; `/spost`; `ShadowsOfUI_PostOfficeDB` (v1). Express not yet ported (see fork port doc). | [ShadowsOfUI-PostOffice/CONTEXT.md](ShadowsOfUI-PostOffice/CONTEXT.md) |
