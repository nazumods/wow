...
LibNAddOn
    ...
    +-- ShadowsOfUI-HousingVendor (LibNAddOn only; hooks the merchant window; no LibNUI)
...
| **ShadowsOfUI-HousingVendor** | Overlays housing-decor icons in the **Merchant** window: an in-storage owned-count badge (bottom-left), a gold first-acquisition-bonus star on unowned decor that grants a one-time House XP bonus (top-right), and an optional owned-check (top-left, off by default). One synchronous `C_HousingCatalog.GetCatalogEntryInfoByItem` call per item, class-gated + session-cached; `MerchantFrame_UpdateMerchantInfo` hook + `HOUSING_STORAGE_UPDATED` refresh + login catalog prime. Native replacement for the at-vendor decor overlays in Ludius Plus / Plumber / RiddleCompass / Decor Vendor; complements ShadowsOfUI-Collectibles' decor tint. `/shvendor` (`itemtest` dev dump). Standalone, no LibNUI. `ShadowsOfUI_HousingVendorDB` (v1). | [ShadowsOfUI-HousingVendor/CONTEXT.md](ShadowsOfUI-HousingVendor/CONTEXT.md) |
...
| ShadowsOfUI-HousingVendor | `/shvendor` | `""` (status: indicator toggles), `itemtest` (dev: dump owned/stored/bonus for the item under the cursor) |
