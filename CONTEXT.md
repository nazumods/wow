...
| Warbandeer_Characters | `/characters`, `/wbc` | `list`, `delete <name>`, `cleanup`, `refresh`, `refresh items/locks`, `dump`, `dump bank/bankgear/gt/locks/artifact/warband/profgear/wq[ self]/m+/vault/delves/dungeons/glyphs`, `wdump [*]` / `wdump <same subcmds>` (copyable-window variant of every `dump`), `clear delves/dungeons`, `stat`, `missing`, `missing me`, `wmissing` |
