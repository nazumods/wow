...
| **ShadowsOfUI-Known** | Headless tooltip addon, two surfaces: a "Learnable by:" block on recipe item tooltips (alts with the profession that haven't learned it; red if skill too low), and a "Known by:" / red "Not Known" line on the **Place Crafting Order** browse list (which characters already know the hovered recipe, matched by captured recipe id). Reads `WarbandeerApi` + optional `WarbandeerDB.profIntent`. `/sknown` dev dumps. No UI, no DB. | [ShadowsOfUI-Known/CONTEXT.md](ShadowsOfUI-Known/CONTEXT.md) |
