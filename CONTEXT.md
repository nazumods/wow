...
| **Warbandeer_Collected** | Transmog set tracker (`/collected`, `/collect`). DataView grid, scan logic, per-set **wanted** flag + **S–F tier ranking** (global baseline w/ per-race overrides) edited from the grid (shift-click) + dressing room, plus a **live/PTR toggle** previewing the upcoming (PTR-only) sets (`data/sets_ptr.lua`, generated by `tools/update-sets.ps1 -PtrDelta`), `WarbandeerCollectedDB` (v3); exposes `WarbandeerCollectedApi` (read scan data + PTR delta + read/write ratings; consumed by Warbandeer's `collected` view). | [Warbandeer_Collected/CONTEXT.md](Warbandeer_Collected/CONTEXT.md) |
