...
| File | Purpose |
|---|---|
| `core.lua` | Init, `PRESETS`/`COLOR_OPTIONS` (7 **bold-but-legible** known-item tints — the tint MULTIPLIES the icon texture, so one channel is held at full for an obvious cast while the others stay clear of zero to avoid crushing to black; green is absent, reserved for `UncollectedColor`), `Defaults` (blue default tint `{0.25,0.45,1.00}`, `knownColor=1`), `MigrateDB` (**v2**, non-destructive: v1→v2 moves only a user still on the retired pure-red factory default to soft blue; any chosen preset or `/scollect custom` colour is preserved), refresher registry (`ns.AddRefresher`/`ns.Refresh`), `ns:settingChanged` (applies the colour preset only when the `knownColor` dropdown changed, so a custom colour survives other toggles), `ns.KnownColor()`, `ns.UncollectedColor`, `RegisterSettings` (5 fields), `ns:RegisterChangelog()` (Changelog button), `/scollect` (status / `custom` colour picker / `itemtest`). Loaded **first** so `AddRefresher` exists before `surfaces.lua` runs. |
...
| `preview.lua` + `preview.xml` | Settings **vendor-card preview**, laid out as a left **controls column** (the wrap-arrow tint stepper + the two sliders) beside a right **preview** — under a **"Preview"** header, a **"Not tinted"** control column paired close (`COLUMN_GAP = 175`) with a captioned **"Tinted"** twin column so each item reads as a normal/coloured pair. Three sample merchant cards per column (common/rare/epic) built from the real merchant textures (`UI-EmptySlot` slot, `UI-Merchant-LabelSlots` name plate, icon + `UI-Quickslot2` border, quality-coloured `C_Item.GetItemQualityColor` name, `GetCoinTextureString` price). The left column is an **untinted control** (never touched by `updateCard`); the right column (`self.rows`) is its **tinted twin**, tinted **whole-card** exactly like `surfaces.lua` tints the live merchant — name plate + slot at full `ns.KnownColor()`, icon + border at 0.9x, icon-only `SetDesaturation`. This **is** the tint picker — there is no dropdown. A back/next **wrap-arrow** stepper (native `common-dropdown-icon-back`/`-next` atlases) `step`s `db.knownColor` ±1 with wrap-around and calls `ns:settingChanged("knownColor")` (applies the preset rgb + `ns.Refresh()`). Two **debug sliders** (`MinimalSliderWithSteppersTemplate`, 0–1, via a shared `makeSlider`): **saturation** fades the item's own content (icon `SetDesaturation` + name-text luminance fade) but NOT the tint wash, and **icon dim** fades the icon via `SetAlpha` (Blizzard's unusable-item look). Both preview-only — not saved, not applied to real tinting. Registered as the **first settings field** (`{ typ = "element", template = ... }`, via LibNAddOn's `element` field type) so it sits at the top of the panel; `preview.xml` is the `virtual` template (`ShadowsOfUI_Collectibles_TintPreviewMixin`, a required global). The card rides `ns.AddRefresher` so it tracks every colour change. |
