...
| `lua/strings.lua` | `ns.lua.strings` — `startsWith(str, start)`, `split(token, str)`, `duration(seconds, style?)`, `stripEscapes(str)` (strip `|c`/`|r` colour, `|T…|t` texture, `|A…|a` atlas; nil-safe), `trim(str)`. **`trim` returns `""` for nil and for whitespace-only, where `stripEscapes` passes nil through** — the difference is deliberate and follows the callers: trim's inputs are user-typed (text boxes, slash args) where absent and blank mean the same thing, and all twelve sites it replaced were already written `(s or ""):match(…)`; stripEscapes' inputs are API strings where absent and empty differ |
| `lua/signal.lua` | `ns.lua.signal()` → a `Signal` with `Subscribe(fn)` (returns fn, so it can be kept for the inverse), `Unsubscribe(fn)` (removes ONE registration, returns whether it removed anything), `Fire(...)`, `Count()`. The pub/sub pattern eight places in the suite had hand-rolled as `list[#list+1] = fn` plus an `ipairs` loop — it recurs wherever one producer can't know its consumers (a shared dressing room whose edits must reach grids in several addons). **Not an event bus**: no named channels, no priorities, no return values — a signal IS the channel, so the owner names it by naming the variable and nothing parses a string at fire time. `Fire` iterates a **snapshot**, so a listener that unsubscribes itself mid-fire can't make the next listener skip its turn; the change lands from the following fire |
...
| `settings.lua` | `ns.registerSettings(addOn, name, features)` — Blizzard Settings API (checkbox/slider/dropdown/**element**, the last a settingless custom-frame initializer positioned in field order). Also the shared category get-or-create helpers `ns.getSettingsParent(name)` (top-level, keyed by name) and `ns.getSettingsSubcategory(parentName, title)` (subcategory by title under a parent — idempotent, unlike `Settings.RegisterVerticalLayoutSubcategory`; spec'd in `settings_spec.lua`) |
| `changelog.lua` | `ns.registerChangelog(addOn, name, parentName?)` — adds a **"Changelog"** button to the addon's settings category (via `CreateSettingsButtonInitializer` + `Settings.RegisterInitializer`, deferred to `ADDON_LOADED`) that opens `addOn.changelog` (a newest-first `{version, notes}` list from the addon's `changelog.lua`, appended at release by `release.sh`) in LibNUI's shared `CopyWindow`. Resolves the addon's own category **get-or-create**, so the button lands once and never adds a second line under the parent: prefers a recorded category (`settingsCategoriesByTitle[_TITLE]` from declarative `registerSettings`, or `settingsCategory` from an imperative panel); otherwise `ns.getSettingsSubcategory(parentName, _TITLE)` reuses an existing subcategory of that title **however it was registered** (so the dedup no longer depends on the addon having recorded a category — the fix for an old or other-built addon), else a top-level category keyed by `_TITLE`. The ShadowsOfUI-* addons pass `"Shadows of UI"`; Warbandeer passes nothing (top-level). Display prefers `addOn.ui`, falls back to the global `LibNUI` if loaded, then chat. Adds `addOn:RegisterChangelog`/`ShowChangelog` |
...
A category with `parent = "<name>"` registers as a Settings **subcategory** under a shared parent group (created+registered once). The parent is keyed by name in a LibNAddOn-global table, so every addon (and every category) using the same name converges on one group. `addOn:GetSettingsParent(name)` exposes the same get-or-create for addons that build their panel another way (e.g. a LibNUI `SettingsFrame:RegisterSubcategory(parent, ...)`). `ns.getSettingsSubcategory(parentName, title)` is the matching get-or-create for **subcategories** (`Settings.RegisterVerticalLayoutSubcategory` is not idempotent), so `RegisterChangelog` reuses an addon's existing subcategory by title however it was registered instead of minting a duplicate line. Assigning the returned category to `addOn.settingsCategory` is now just an optimization — a recorded category is preferred, but no longer required to avoid the duplicate.
