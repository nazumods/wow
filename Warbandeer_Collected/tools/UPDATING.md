...
weapon/off-hand **appearances on the PTR (`wowt`) but not yet on live (`wow`)** — muted-blue markers
(a count on a PTR client, a dot on a live realm; gated by `ns.OnPtr`), with a `+N appearances upcoming ·
PTR <build>` header tally. That delta lives in
