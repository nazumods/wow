...
if ($AuditSets) {
  ...
  # against real data). $haveByAppr maps each rendered appearance signature to one of its set
  # ids, so an un-rendered set with the IDENTICAL look is flagged a duplicate and paired to it.
  $have = @{}
  ...
  $haveByAppr = @{}
  foreach ($id in $have.Keys) { if ($imaKey.ContainsKey($id)) { $k = $imaKey[$id]; if (-not $haveByAppr.ContainsKey($k)) { $haveByAppr[$k] = $id } } }
  ...
  $L.Add('-- EPHEMERAL — generated by tools/update-sets.ps1 -AuditSets. DO NOT COMMIT a')
  $L.Add('-- populated copy; git checkout this file when done. The committed version is empty.')
  $L.Add('-- Category -> Review : wago sets not yet rendered (genuine gaps to triage).')
  $L.Add('-- Category -> Twins  : TRUE appearance-duplicates (identical source items to a rendered')
  $L.Add("--                      set) — original + duplicate paired for an in-game A/B check.")
  $L.Add('')
  $total = 0
