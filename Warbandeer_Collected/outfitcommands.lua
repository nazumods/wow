...
ns:registerCommand("outfit", nil, function(_, args)
  ...
  -- The one verb here that needs no room: it produces a look rather than acting on the one on
  -- screen, and dresses the preview only as a bonus when it happens to be open (#819).
  elseif sub == "inspect" then ns.GrabTargetOutfit()
  ...
  elseif sub == "push" then outfitPush(rest)
    ...
    ns.Print("                       | export | post | import <string or link> | inspect")
    ...
    ns.Print("  inspect — read your TARGET's transmog as a /customset string (copy window)")
  end
