...
The **PTR PREVIEW** title-bar button surfaces the **upcoming** sets at the top of
the grid — the transmog sets that are on the current Public Test Realm but **not yet
on live** — pushing your normal live sets down beneath them, so you see what's coming
without losing your collection view. Each upcoming set shows a muted blue dot (there's
no collection data for content that isn't out yet), and the header adds a "+N
upcoming" tally with the PTR build the preview was built from. You can still **Shift-click to flag** an upcoming set as
