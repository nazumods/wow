...
| `controls/DressingRoom.lua` | `ns.DressingRoom` (TitleFrame, `special`, `strata=HIGH`) + `ns.ShowDressingRoom(group, set)` / `ns.HideDressingRoom()` — persistent 3D preview: a `ui.Model` flanked by paper-doll equipment-slot columns (built/refreshed in the companion `DressingRoomSlots.lua`), a class-themed model backdrop (`_setBackground` — `dressingroom-background-<class>` atlas keyed to the set's class, behind the transparent ModelScene) with a **Background** toggle (`SetBackgroundOn`, shares the top control row with Undress, defaults on), plus an Undress toggle (`SetUndressed` — bare body for race id), a gender toggle, a wrapped race-icon selector (defaults to the logged-in char; each icon has a `GameTooltip` naming the race), a **form toggle** (`SetForm`/`_setupForms`/`_highlightForm`) — a small floating button row at the model's top-left, shown only for multi-form races (Worgen/Dracthyr) whose `ns.RaceModels` entry has `forms`; a reusable 2-button pool, hidden for single-form races — and a **scale slider** (`ui.Slider`) at the model's bottom-left with a readout: drives `Model:Scale` live (frame-level lifted above the model so the thumb is grabbable), reset to the race's `scale` by `Dress`; dev `/collected scale <n>` drives it too via `ns.PreviewModelScale`. Title bar carries prev/next set-navigation arrows + a class icon: `Step(dir)` cycles the current group's class sets (wraps, skipping the empty `{}` class slots), and `_load(group, set)` is the shared "preview this set" path (title + class icon + `UpdateSlots` + `Dress`) used by both `Step` and `ns.ShowDressingRoom`. `Dress()` resolves the display ID off the `ns.RaceModels` entry (multi-form → `entry.forms[_form][sex]`, else `entry[sex]`) and uses `Model:DisplayInfo` when one exists (exact race+gender), else renders the race via `Model:Unit("player", raceID)` (textured, gender follows the char), then `TryOn`s every `C_TransmogSets.GetAllSourceIDs(set.id)` source. `ns.PreviewModelID(id)` forces a raw display id into the open model (dev `/collected model`) |
