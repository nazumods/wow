...
| Great Vault | Every **max-level** character's Great Vault at a glance (the Great Vault is a max-level feature, so levelling characters are omitted): how many of the three reward slots on each track (Raid, Dungeons, World) are unlocked — shown as pips, with the exact reward item level per slot on hover — plus your owned keystone and this week's raid lockout (hover for the full list). Uses the same Alliance / Horde / Both faction control as Summary, and marks any character with a reward still waiting to be collected |
