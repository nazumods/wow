...
| Great Vault | Every character's Great Vault at a glance: how many of the three reward slots on each track (Raid, Mythic+, World) are unlocked — shown as pips, with the exact reward item level per slot on hover — plus your owned keystone, this week's raid lockout (hover for the full list), and whether the weekly Delver's Bounty is claimed. Uses the same Alliance / Horde / Both faction control as Summary, and marks any character with a reward still waiting to be collected |
