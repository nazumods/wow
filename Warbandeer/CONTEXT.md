...
| `controls/UpgradeMark.lua` | `ns.UpgradeMark(charName, slot)` → cell-text ▲ glyph (green held / gold better-in-warband), `ns.UpgradeTip(charName, slot)` → hover line, and `ns.UpgradeSuggestion(charName, slot)` → `link, ilvlGain, warband` for the upgrade item (drives Detail's per-slot sub-line); all via `ShadowsOfUI_UpgradeApi` (OptionalDep); no-op "" / nil when that addon is absent |
...
| `views/summaryCol/*.lua` | One file per Summary column: faction, role, character, level, ilvl, upgrades, catchup, profs, bags, vault, keystone, crests, catalyst, voidcore, delves, lumber, cofferKey, caches, xp, rested, played, gold. `catchup.lua` (the "<180" column) counts Head/Neck/Hands/Waist slots below ilvl 180 for level-81+ characters (muted em-dash below 81; green check at 0). `xp.lua` shows current XP as a percent of the level's total (hidden at max level). `upgrades.lua` (the "Up" count column) early-returns unless `ShadowsOfUI_UpgradeApi` exists, so its column is only registered when ShadowsOfUI-Upgrade is loaded |
