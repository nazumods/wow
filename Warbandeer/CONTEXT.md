...
| `controls/SuggestedBox.lua` | `ns.SuggestedBox` — Detail's left-column "SUGGESTED" panel: a priority-ordered (ilvl-gain desc) list of ready gear upgrades the character can equip **right now**, from `ShadowsOfUI_UpgradeApi:CharacterUpgrades` (held green / warband gold). `Populate(char)` (caller sets width first) fills ≤5 pooled rows — `{icon} [Item] +N ilvl {Slot}` — filtering out items whose required level exceeds the character's; row hover lays the upgrade beside the equipped piece via `ns.ShowItemTooltip`. Ready upgrades come first; any remaining rows list active **world-quest** gear rewards that would upgrade a slot (`ShadowsOfUI_UpgradeApi:WorldQuestUpgrades`, orange, `{icon} [Item] +N ilvl WQ: <title> — <zone>`) — a future action vs. the green/gold immediate ones; **clicking a WQ row opens the world map to the quest's zone (`mapID`) and super-tracks it**. After the world-quest rows, any still-remaining rows list **faction-quartermaster** gear that would upgrade a slot (`ShadowsOfUI_UpgradeApi:VendorUpgrades`, cyan, `{icon} [Item] +N ilvl Buy: <quartermaster> (<zone>) — <cost>`) — a purchase action; **clicking a vendor row opens the world map to the quartermaster's zone (`mapID`)** (no quest to super-track). Both future-action sources degrade silently on an older Upgrade addon without the respective method. Shows "Nothing pressing — looking good!" when there are none; hides + returns 0 height when ShadowsOfUI-Upgrade (OptionalDep) is absent |
