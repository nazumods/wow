...
| `views/summaryCol/*.lua` | One file per Summary column: faction, role, character, level, ilvl, upgrades, profs, bags, vault, keystone, crests, catalyst, voidcore, delves, lumber, cofferKey, caches, xp, rested, played, gold. `xp.lua` shows current XP as a percent of the level's total (hidden at max level). `upgrades.lua` (the "Up" count column) early-returns unless `ShadowsOfUI_UpgradeApi` exists, so its column is only registered when ShadowsOfUI-Upgrade is loaded |
