...
| `views/summaryCol/*.lua` | One file per Summary column: faction, role, character, level, ilvl, upgrades, enchants, catchup, profs, bags, vault, keystone, crests, catalyst, voidcore, delves, lumber, cofferKey, caches, xp, rested, played, gold. `catchup.lua` (the "<180" column) counts Head/Neck/Hands/Waist slots below ilvl 180 for level-81+ characters (muted em-dash below 81; green check at 0). `xp.lua` shows current XP as a percent of the level's total (hidden at max level). `upgrades.lua` (the "Up" count column) and `enchants.lua` (the "Ench" column — count of equipped slots missing a permanent enchant, green check at 0, muted em-dash below max level; hover lists slot + item + the recommended enchant via `ns.RecommendedEnchant`, click → Detail) both early-return unless `ShadowsOfUI_UpgradeApi` exists, so those columns are only registered when ShadowsOfUI-Upgrade is loaded |
