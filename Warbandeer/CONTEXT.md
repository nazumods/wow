...
| `views/DetailView.lua` | Single-character detail: level/race/class/realm/ilvl + per-profession intent editor. `BuildFilter` character-picker dropdown; writes `profIntent` via `ns.data.SetProfIntent`. `Select(toon)` switches the displayed character (used by the picker and by clicking a Top Characters row on the Overview) — updates the picker label, calls `OnBeforeShow`, refits |
