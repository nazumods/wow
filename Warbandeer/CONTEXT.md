...
| `controls/UpgradeMark.lua` | `ns.UpgradeMark(charName, slot)` → cell-text ▲ glyph (green held / gold better-in-warband), `ns.UpgradeTip(charName, slot)` → hover line, `ns.UpgradeSuggestion(charName, slot)` → `link, ilvlGain, warband` for the upgrade item (drives Detail's per-slot sub-line), and `ns.MissingEnchantSlots(charName)` → `{ [slot]=true }` set of equipped slots missing a permanent enchant (built once per Detail render, drives the gear-row "Missing enchant" note); all via `ShadowsOfUI_UpgradeApi` (OptionalDep); no-op "" / nil / empty-set when that addon is absent |
