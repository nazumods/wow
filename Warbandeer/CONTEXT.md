...
| `views/summaryCol/*.lua` | One file per Summary column: faction, role, character, level, ilvl, upgrades, enchants, gems, catchup, profs, bags, vault, keystone, crests, catalyst, voidcore, delves, lumber, cofferKey, caches, xp, rested, played, gold. `catchup.lua` (the "<180" column) counts Head/Neck/Hands/Waist slots below ilvl 180 for level-81+ characters (muted em-dash below 81; green check at 0). `xp.lua` shows current XP as a percent of the level's total (hidden at max level). `upgrades.lua` (the "Up" count column) and `enchants.lua` (the "Ench" column — count of equipped slots missing a permanent enchant, green check at 0, muted em-dash below max level; hover lists slot + item + the recommended enchant via `ns.RecommendedEnchant`, click → Detail), and `gems.lua` (the "Gem" column — total empty gem sockets across equipped gear via `MissingGems`, green check at 0, em-dash below max; hover lists slots + the recommended gems via `ns.RecommendedGems` — the unique diamond "×1" then the fill gem) all early-return unless `ShadowsOfUI_UpgradeApi` exists, so those columns are only registered when ShadowsOfUI-Upgrade is loaded |
...
| `views/detail/Gear.lua` | DetailView equipped-gear row pool: `_gearRow` (slot icon + name + ilvl + track + upgrade sub-line), `_showGear` |
| `views/detail/Stats.lua` | DetailView secondary-stat 2×2 grid (Crit + Mastery / Haste + Versatility) under the stat cards: `_buildStatGrid` (lazy, 4 module-bg cells) + `_showStats` (fills `pct`/`rating` from the data layer's `stats.secondary`, blanks `—` when unscanned). The spec's tier-1 stat(s) are tinted gold via `ShadowsOfUI_UpgradeApi:StatRanks` (OptionalDep). When `:StatTargets` returns an Archon target the rating shows as "current / target" with the current `deltaCode`-coloured (±5% green = on-target, blue = over, red = under) |
