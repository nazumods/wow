...
| `tooltip.lua` | `AddTooltipPostCall(Item)` → lowercases the tooltip lines, `ns.FactionIDByName` matches a tracked faction, appends the block (item tooltips auto-show, no re-Show). Resolution is **memoized per `data.id`** (`false` caches a no-faction result), wiped whenever `ns._factionIndexGen` (bumped at the end of `RebuildFactionIndex`) changes — item tooltips re-fire constantly, so the repeat cost drops to a table lookup. `/sreps` dev/lookup command. |
