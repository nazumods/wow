...
| `data/achievements.lua` | Account-wide (not a broker): `db.achievements` snapshot of every catalog-tracked id's `{completed, wasEarnedByMe}` + the account's `totalPoints`. `ns:CaptureAchievements`/`InitAchievements` (login) re-snapshot on `ACHIEVEMENT_EARNED`. **This snapshot only ever grows within a build (#733):** `completed` is written as `live or stored`, never from the live read alone — on a cold login the achievement API answers `false` indistinguishably from a genuine negative, and this store is account-wide, so one bad login would blank every character's answer. `totalPoints` is only written when the live read is `> 0`, for the same reason (points are monotonic, so the guard masks nothing). `wasEarnedByMe` is deliberately NOT sticky — it is per-character last-captured-wins, and OR-ing it would silently change that to any-character-wins. The points write sits ABOVE the tracked-id guard (#745-1), since the total is account-wide and moves for any achievement earned. Names are NOT persisted — the addon resolves them live via `GetAchievementInfo`; a desktop-facing name catalog is deferred to #639. `wasEarnedByMe` is genuinely per-character but captured account-wide from whichever character last triggered a snapshot (deliberate simplification, matching `data/quests.lua`'s `CatalystUnbound`). `WarbandeerApi:GetAchievement`/`IsAchievementComplete`/`GetTotalAchievementPoints`; `/wbc dump achievements` |
...
| `data/professions_gear.lua` | The broker's `gear` field (profession tool/accessory slots, keyed by parent skillLineID; each slot record carries `name`/`link`/`ilvl`/`rarity`/`tier`/`expacID`/`equipLoc`, every field preserved from the previous scan whenever the API hands back nil) + its equip-load fallback (`anyUnloaded`/`scheduleFallback`, generation-guarded) + the `ITEM_DATA_LOAD_RESULT` drain that refreshes once profession-slot item data loads. Adds `ns.Professions.fields.gear` to the broker in `data/professions.lua` |
| `data/recipegear.lua` | Account-wide `db.recipeGear` cache (recipe → prof-gear output: itemID/rarity/equipLoc/target skillID/output item `name`), build-stamped; the `name` is captured at resolve time (the item is warm by then) because consumers key a gear line by the name's last word — entries cached before the field existed are backfilled in place on the next `ResolveRecipeOutput` rather than invalidating the build stamp; **also `locale`-stamped (#745-5)** — `name` comes from `C_Item.GetItemInfo` and is locale-dependent, so a language switch inside one build kept foreign names and silently broke the family match. A locale change clears only the `name` fields (itemID/equipLoc/classID/subClassID are locale-independent and expensive to re-resolve) and lets the lazy backfill re-resolve them; a cache with no `locale` key takes that path once, self-healingly. Additive, so no DB version bump; `WarbandeerApi:ResolveRecipeOutput`, `WarbandeerApi:ClassifyProfGearItem` (item → prof skillID/equipLoc, shared with the bank scanner), `WarbandeerApi:ClassifyGearItem` (item → equippable equipLoc/classID/subClassID, shared by `gearbag`/`bank`) |
...
| `data/titlecatalog.lua` | Account-wide **title catalog** at `db.titleCatalog` — the whole player-title universe (**earned AND unearned**) as `names[titleMaskID] = trimmed display name`, plus `accountWide[titleMaskID] = true`. Account-wide, so it lives once (the `data/travelerslog.lua` pattern) rather than per character. Exists because the two things an offline reader can't get from the per-character `titles.known` cache are exactly these: **names for titles nobody has earned**, and the account-wide flag (`CharTitles.db2` has no account-wide column and Blizzard REST has no equivalent — see #633 / #639). Names follow the **client locale** (a wago `Name_lang` extract is enUS-only), so `locale` is recorded beside them, with `build` + `scannedAt` + `scannedBy`. `ns:InitTitleCatalog()` (from `initialize()`, after `InitAchievements`) takes a scan and re-scans on `KNOWN_TITLES_UPDATE`, debounced 1s (matching the `titles` broker's `eventDelay`). Scan = `1..GetNumTitles()` **without** the `IsTitleKnown` gate (so the full universe), keeping ids whose `GetTitleName` 2nd return says real player title; `accountWide` is the scanning character's `IsTitleKnown` — the same oracle Warbandeer's live Titles view uses, hence ONE character's observation, not a proven property. Guard: a scan returning zero rows (pre-data login) keeps the last catalog rather than blanking it. Pure shaper `ns.ShapeTitleCatalog(rows, meta)` is unit-tested (`spec/titlecatalog_spec.lua`) and rebuilds both tables from scratch each scan, so a removed title vanishes with no stale keys. **No DB version bump** — purely additive and lazily filled, so rollback is lossless. Drives `WarbandeerApi:GetTitleCatalog`; reported by `/wbc dump titles` |
| `data/classcollectibles.lua` | Account-wide **class-collectible ownership** snapshot at `db.classCollectibles` — the two oracles Detail's appearance cards resolve LIVE each render, frozen for a reader with no client: the appearance-unlock quest flags behind `ns.AppearanceUnlocks` (`C_QuestLog.IsQuestFlaggedCompletedOnAccount`) and class-mount `isCollected` behind `ns.ClassMounts` (`C_MountJournal`). Per-character `glyphs.applied` / `glyphs.unlocks` are already persisted by `data/glyphs.lua`; these two weren't, so the offline app couldn't tell which appearance unlocks the warband owns. Neither is offloadable to static game data — `QuestV2` knows a quest exists but never who completed it, `Mount.db2` gives identity not ownership, and the only external answer (Blizzard's account collections endpoint) needs an OAuth client secret no distributed binary can ship (see #636 / #639). Account-wide, so it lives once (the `data/titlecatalog.lua` pattern) and **both catalogs are walked for every class**, not just the logged-in one. Unlocks are keyed by **quest id**, not one bool per catalog entry, because a progressive row (green fire) needs `startQuest` answered alongside `quest` to pick which itemID to surface — the quest-keyed set reproduces `GetAppearanceUnlocks` exactly. Mount ownership is **tri-state**: `true` collected, `false` resolved-but-not-collected, **absent** when the id didn't resolve to a mountID that scan — `GetMountFromItem` reads nil for a cold item, so a first-login scan records "unknown", never a false negative — and a `false` is only trusted when the journal is actually populated (`GetNumMounts() > 0`), else the entry stays absent. **This snapshot only ever grows within a build (#733):** both halves are monotonic, so `scanRows` ORs each row against the stored snapshot rather than replacing it — the account quest payload can be cold while the mount journal is warm, and the old empty-journal bail tested the wrong half, so one bad login on any alt blanked the whole warband's answer. That bail is now just a cheap early-out. `ShapeClassCollectibles` stays a pure transform (its specs are unchanged); stale keys are still dropped, since only ids the current catalog declares are ever emitted. `ns:InitClassCollectibles()` (from `initialize()`, after `InitTitleCatalog`) scans at login and re-scans on `NEW_MOUNT_ADDED` / `QUEST_TURNED_IN` / `ITEM_DATA_LOAD_RESULT` (filtered to catalog itemIDs — that last one is what clears an `unresolved` mount without a `/reload`), all sharing one 1s-debounced trailing scan. The `ITEM_DATA_LOAD_RESULT` hook checks the event's `success` payload and caps each id at 5 attempts before dropping it from the watch set (#733) — without either, an id that never resolves drove a permanent ~1 Hz snapshot rebuild for the whole session. Guard: bails keeping the last snapshot while `C_MountJournal.GetNumMounts()` reads 0 (pre-data login). Pure shaper `ns.ShapeClassCollectibles(rows, meta)` is unit-tested (`spec/classcollectibles_spec.lua`) and rebuilds every table from scratch each scan, so a dropped id vanishes with no stale keys. **No DB version bump** — purely additive and lazily filled, so rollback is lossless. Drives `WarbandeerApi:GetClassCollectibles`; reported beside live by `/wbc dump glyphs` (unlock half) and `/wbc dump mounts` (mount half), which flag any row where stored and live **disagree** |
| `data/travelerslog.lua` | Account-wide **Traveler's Log** (Trading Post monthly activity) snapshot — the activity bar + Trader's Tender are warband-shared, so it lives once at `db.travelersLog` (the `data/warband.lua` account-wide pattern), not per character. `ns:InitTravelersLog()` (from `initialize()`, after `InitWarband`) seeds the store, requests pending chest rewards (async), takes an initial read, and hooks `PERKS_ACTIVITIES_UPDATED` / `PERKS_ACTIVITY_COMPLETED` / `CHEST_REWARDS_UPDATED_FROM_SERVER` / `PERKS_PROGRAM_CURRENCY_REFRESH`; a shared `refresh()` re-derives a compact snapshot via `derive(info, pending, currency)` (Blizzard's `Blizzard_MonthlyActivities.lua` formula — earned = sum of completed activities' contribution capped at the largest threshold; pct = earned/max; rewards joined from pending chest rewards by `thresholdOrderIndex`, filtered to the active month; `count` is every pending chest so a prior month's uncollected reward still flags). Each pending reward item's `name`/`icon` is captured alongside the id list in `rewards.itemInfo` (the id list itself is unchanged), so the Overview reward strip is renderable without a client; nil-tolerant, since the whole snapshot is re-derived on every Perks event. Guards: bails keeping the last snapshot when `C_PerksActivities` is absent or `thresholds` empty (Trading Post disabled / pre-data). Drives `WarbandeerApi:GetTravelersLog` → Warbandeer's Overview **Traveler's Log** strip. `/wbc dump travelerslog` (+ `wdump travelerslog`) prints the snapshot |
...
| `data/stats.lua` | Broker `stats`: `secondary` (v16) — `{ crit, haste, mastery, versatility }`, each `{ pct, rating }` (effective % + gear combat rating) from the live `GetCritChance`/`GetHaste`/`GetMasteryEffect`/`GetCombatRatingBonus` + `GetCombatRating` APIs (logged-in char only), so it's a last-seen snapshot read warband-wide. `mastery` also carries its active-spec passive `spell` id (`GetSpecializationMasterySpells`) and that spell's `description`, letting Detail name the mastery + show its spec-specific effect. The description is stored (unlike the name, which is static `SpellName.db2` data — see #639) because `GetSpellDescription` substitutes the *logged-in* character's own mastery coefficient into the template, so it is character state that can't be re-derived from the spell id; it is re-read on each `COMBAT_RATING_UPDATE` so the figure tracks current mastery. `GetSpellDescription` answers `""` until the spell's data loads — well after the login refresh runs — so a cold read requests the load (`C_Spell.RequestLoadSpellData`) and keeps the last good text for that same spell instead of blanking it; `SPELL_DATA_LOAD_RESULT` then re-scans the field once, gated on the pending spell id so a failed load can't drive a request/rescan loop. Refreshes on `COMBAT_RATING_UPDATE`. Drives Warbandeer's Detail stat grid. **Secret values:** these APIs can return WoW "secret" numbers (tainted addon code can't do arithmetic on them); each is passed through `safe()` (`canaccessvalue` guard, retail-only) and stored as **nil** when secret, so the snapshot is always safe to read + serialize (Detail's delta/radar math would otherwise taint) |
...
| `missing.lua` | `/wbc missing`, `/wbc missing me`, `/wbc missing audit` — a generic **walker** over every broker field's `missing` descriptor + every `ns:RegisterMissing` provider (self-registering: a field is *tracked* by a descriptor, opted out by `missing = false`, or auto-flagged when undeclared, so a new field participates for free). Emits labels ordered by each entry's explicit `order`; `ns:getMissingReport` pins the current character atop the alpha-sorted rest. `ns:AuditMissing` returns `(undeclared, collisions)`: broker fields lacking a stance, plus `"order N: a, b"` lines for any explicit `order` shared by >1 entry (fields + providers) — the #745-3/#922 duplicate-order bug that nothing else asserts. Both surfaced by `/wbc missing audit` and a silent load-time dev nudge; the mechanism is locked by `spec/missing_spec.lua` (the real data can't load under busted, so uniqueness of the live map is caught in-game, not in CI). Hosts only the external `bars` provider (optional `WarbandeerBarsApi`, no data-file home) |
...
WarbandeerApi:ResolveRecipeOutput(recipeID) → RecipeGearInfo|false|nil
    -- what prof gear a recipe crafts ({itemID, rarity, equipLoc, skillID, name?}),
...
WarbandeerApi:GetBankProfGear(skillID)     → BankGearEntry[]
    ...
    -- character, and guild ({itemID, equipLoc, rarity, count, name?, source,
    -- sourceType}); `name` is captured at scan time from GetItemInfo(itemID), with
    -- the slot's own link as a cold-cache backstop — crafted prof gear carries its
    -- quality-tier atlas inside the link's brackets, so `linkName` strips inline
    -- escapes (an unstripped name would key the gear line on the atlas, not the
    -- Apron/Toolbox/Hammer noun).  source is a display label, sourceType ∈ warband/character/
...
WarbandeerApi:GetClassCollectibles()       → ClassCollectibles?
    -- the ACCOUNT-WIDE snapshot behind the two getters above (no char argument), for a reader with no
    -- client: quests[questID] = true for every AppearanceUnlocks quest the account has completed —
    -- both a row's `quest` AND a progressive row's `startQuest`, so the next-step itemID choice
    -- reproduces exactly — plus mounts.spells[spellID] / mounts.items[itemID] for ClassMounts, TRI-STATE
    -- (true collected / false resolved-not-collected / ABSENT = the id didn't resolve that scan, so a
    -- cold item reads unknown rather than a false negative); + build / scannedAt / scannedBy and the
    -- questCount / mountCount / mountUnresolved sizes.  In game prefer the live getters above.  nil
    -- until any character has logged in since this shipped — issue #636
...
WarbandeerApi:GetTitleCatalog()            → TitleCatalog?
    -- the ACCOUNT-WIDE title universe (no char argument): names[titleMaskID] = trimmed display name for
    -- every valid player title, earned AND unearned, in the CLIENT LOCALE; accountWide[titleMaskID] = true
    -- for the ids `scannedBy` knows; plus build / locale / scannedAt / count / accountWideCount.  accountWide
    -- is ONE character's IsTitleKnown observation, not a proven property — cross-reference the other
    -- characters' GetTitles(char).known sets to tighten it.  nil until any character has logged in since
    -- this shipped.  Exists so an offline reader can name an UNEARNED title (per-character titles.known
    -- already carries the earned ones) — issue #633
...
reputations = {                                         -- v21; per-faction standings (broker)
  factions = { [factionID] = {                          -- every faction with a standing
    ...
    current?, threshold?,                                -- absolute progress within the current rank/level (bar-fill numerator/denominator: renown XP / friendship or reaction points past the tier floor); nil when there's no next tier (e.g. a maxed standard faction)
    paragonCurrent?, paragonThreshold?,                  -- progress within the current paragon reputation-bag cycle, when paragon is true
  } }?,
}?
gearbag = {
...
{ version = 41, numCharacters, lastDailyReset, lastReset, lastSundayReset,
  ...
  travelersLog = {
    ...
    rewards = { count, pendingTender, items = { itemID },    -- uncollected chests waiting + Tender/items
              itemInfo = { [itemID] = { name?, icon? } } },-- display metadata for the reward strip,
                                                           -- captured at refresh time; absent on
                                                           -- snapshots taken before the field existed
  },
  -- account-wide title catalog (no version bump — additive, lazily filled); self-seeded by
  -- data/titlecatalog.lua at login + on KNOWN_TITLES_UPDATE.  The whole title universe, earned AND
  -- unearned; per-character `titles.known` already names the EARNED ones (see #633)
  titleCatalog = {
    build, locale, scannedAt, scannedBy,   -- client build, name locale, server time, WHOSE accountWide this is
    names = { [titleMaskID] = "Name" },    -- trimmed display name, CLIENT LOCALE (not enUS)
    accountWide = { [titleMaskID] = true },-- ids `scannedBy` IsTitleKnown; absent = not known (never false)
    count, accountWideCount,               -- sizes of the two tables above
  },
  -- account-wide class-collectible ownership (no version bump — additive, lazily filled); self-seeded
  -- by data/classcollectibles.lua at login + on NEW_MOUNT_ADDED / QUEST_TURNED_IN /
  -- ITEM_DATA_LOAD_RESULT.  The two oracles Detail's appearance cards read LIVE, frozen for an
  -- offline reader (see #636); in game prefer GetAppearanceUnlocks / GetClassMounts
  classCollectibles = {
    build, scannedAt, scannedBy,           -- client build, server time, which character scanned
    quests = { [questID] = true },         -- AppearanceUnlocks quest AND startQuest ids the account
                                           -- has completed; absent = not completed (never false)
    mounts = {                             -- ClassMounts ownership, keyed by the id the catalog uses
      spells = { [spellID] = true },       -- TRI-STATE: true collected, false resolved-not-collected,
      items  = { [itemID]  = true },       -- ABSENT = id didn't resolve that scan (cold item = unknown)
    },
    questCount, mountCount, mountUnresolved,  -- completed quests, collected mounts, unresolved ids
  },
