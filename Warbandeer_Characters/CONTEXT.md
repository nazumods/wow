...
**Deps:** LibNAddOn, LibNUI · **SavedVars:** `WarbandeerCharDB` (v34) · **Commands:** `/characters`, `/wbc` · **API:** `WarbandeerApi`
...
| `main.lua` | `ns:refresh` + `ns:refreshQueue` (one **field** scanned per 100ms); `/characters refresh`, `/characters dump`. Also `ns:registerDump(subcmd, title, description, body)` — the shared dump-buffer helper: each `body(self, out, args)` writes lines via `out:line(...)` (space-joined varargs) instead of printing, and the helper wires up **both** `dump <subcmd>` (→ chat) and `wdump <subcmd>` (→ `ui.ToggleCopyWindow(title, …)`, re-run toggles). Bare `/wbc wdump` (or `wdump *`) renders every registered dump under `== <title> ==` section headers into one copy window (chat truncates + can't be copied). `args` passes straight through, so a subcommand can take an arg (e.g. `dump wq self`) and the `wdump` variant gets it for free |
...
WarbandeerApi:GetAppliedGlyphs(char?, specID?) → { itemID, label, glyph, applied }[]?, scanned
...
WarbandeerApi:GetAppearanceUnlocks(char?)  → { itemID, label, spell, unlocked }[]?
...
WarbandeerApi:GetLearnedTomes(char?)       → { itemID, label, spell, known }[]?
    -- learned class tomes for the character (currently the Druid "Tome of the Wilds" set):
    -- the catalog (ns.LearnedTomes) merged with the char's known-tome set.  per-character
    -- LEARNED spells (not per-spec); last-seen.  nil for every class but Druid
```
...
glyphs = {                                             -- v33/v34; broker (data/glyphs.lua)
  ...
  tomes   = { [spell] = true }?,                       -- v34; learned class-tome spell ids (Druid Tome of the Wilds); per-character (not per-spec); nil for a class with no tomes
...
| `glyphs` | applied, tomes | `SPELLS_CHANGED`, `PLAYER_SPECIALIZATION_CHANGED` (1000ms debounce) | — |
...
```lua
{ version = 34, numCharacters, lastDailyReset, lastReset, lastSundayReset,
