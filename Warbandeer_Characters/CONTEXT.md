...
**Deps:** LibNAddOn, LibNUI · **SavedVars:** `WarbandeerCharDB` (v33) · **Commands:** `/characters`, `/wbc` · **API:** `WarbandeerApi`
...
| `data/glyphinfo.lua` | The static appearance-glyph catalog (loaded before the broker): `ns.AppearanceGlyphs` (per-character APPLIED cosmetic glyphs, keyed by classId → `{itemID, glyph, label, specs?}`; `glyph` is the id embedded in an applied spell's hyperlink, `specs` the usable spec set) + `ns.AppearanceUnlocks` (ACCOUNT-WIDE Druid Marks/travel glyphs + Warlock demon Grimoires, keyed by classId → `{itemID, quest, spell, label}`). Ids ported from the community GlyphList addon and verified in-game via `/wbc dump glyphs`. Also the pure `ns.MergeGlyphStatus(list, specID, applied)` helper (unit-tested in `spec/`) |
| `data/glyphs.lua` | Broker `glyphs`: `applied` — the set of cosmetic glyph ids the character has applied, keyed by spec id (`{[specID]={[glyph]=true}}`). Walks the class + active-spec spellbook and pulls each known spell's applied glyph id from its hyperlink (`link:match("%b::(%d+)")`, mirroring the GlyphList addon); only the logged-in character's active spec is readable, so it's a last-seen per-spec snapshot. Event `SPELLS_CHANGED`/`PLAYER_SPECIALIZATION_CHANGED` (1s debounce). Also owns the `/wbc dump glyphs` (+ `wdump glyphs`) probe — prints each catalog entry's stored label beside its live-resolved item name (a wrong id shows a mismatch) plus applied/unlocked state. Account-wide unlocks are NOT stored here (read live by the API). Drives `WarbandeerApi:GetAppliedGlyphs`/`GetAppearanceUnlocks` → Warbandeer's Detail appearance card |
...
WarbandeerApi:GetAppliedGlyphs(char?)      → { itemID, label, glyph, applied }[]?, scanned
    -- applied cosmetic appearance glyphs for the character's ACTIVE spec: the class
    -- catalog (ns.AppearanceGlyphs) merged with the char's last-seen applied-glyph set.
    -- per-character + per-spec (last-seen; only the active spec's spellbook is readable).
    -- nil for a class with no glyphs (Evoker); second return `scanned` is false when that
    -- spec has never been scanned.  Drives Warbandeer's Detail appearance card
...
glyphs = {                                             -- v33; broker (data/glyphs.lua)
  applied = { [specID] = { [glyph] = true } }?,        -- cosmetic glyph ids applied, per spec; last-seen (active spec only)
...
| `glyphs` | applied | `SPELLS_CHANGED`, `PLAYER_SPECIALIZATION_CHANGED` (1000ms debounce) | — |
...
```lua
{ version = 33, numCharacters, lastDailyReset, lastReset, lastSundayReset,
