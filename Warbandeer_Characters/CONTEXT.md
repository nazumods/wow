...
**Deps:** LibNAddOn, LibNUI · **SavedVars:** `WarbandeerCharDB` (v31) · **Commands:** `/characters`, `/wbc` · **API:** `WarbandeerApi`
...
| `database.lua` | `ns:MigrateDB` (v31), `ns:initialize` (creates the per-char struct, then `InitBrokers`/`InitWarband`); `/characters list`, `/characters delete <name>` (also prunes the character's cached bank gear), `/characters cleanup` (recount numCharacters + drop bank gear for untracked characters) |
...
| `data/dungeontimes.lua` | Event tracker (not a broker): personal **Mythic+** completion-time cache, structurally mirroring `data/delvetimes.lua` but keyed on keystone level and driven by the clean `CHALLENGE_MODE_START` → `CHALLENGE_MODE_COMPLETED` pair (M+ has an authoritative start event; normal/heroic dungeons have none and are **deferred** to a separate tracker). `CHALLENGE_MODE_START` (re)stamps the authoritative start (after the countdown; does not re-fire on `/reload`); `PLAYER_ENTERING_WORLD` resumes the persisted timer or discards it once `C_ChallengeMode.IsChallengeModeActive()` is false (a run whose start was never seen is not begun mid-run, so partials don't skew the average); `CHALLENGE_MODE_COMPLETED` records `GetServerTime()` elapsed (gated on an in-progress timer). Name via `GetInstanceInfo`, keystone level via `C_ChallengeMode.GetActiveKeystoneInfo` (level 0 = unknown bucket). Stores a rolling window (`KEEP` = 10) of seconds per dungeon+level in `db.characters[name].dungeonTimes`, keyed by `ns.NormalizeDelveKey`, plus the same per-run XP window as delves (moot at max level, where all M+ is played). Drives `WarbandeerApi:GetDungeonStats`. `/wbc dump dungeons`, `/wbc clear dungeons` |
...
WarbandeerApi:GetDungeonStats(dungeonName, level?) → { name, classKey, avg, count, scope, avgXp?, xpCount? }[]?
    -- M+ sibling of GetDelveStats: per-character average keyed-run completion time for one
    -- dungeon, keyed by keystone level.  scope = "+<level>" when that level has samples, else
    -- "all"; avgXp/xpCount as above (nil for max-level M+).  Only keyed (CHALLENGE_MODE) runs
    -- are timed — normal/heroic dungeons are tracked separately (deferred).  Data from the
...
dungeonTimes = {                                        -- v31; event tracker (data/dungeontimes.lua), per character
  active = { key, name, level, start, xp },             -- in-progress M+ run (CHALLENGE_MODE start→complete); nil between runs
  runs   = { [normalizedKey] = {                        -- mirrors delveTimes; key via ns.NormalizeDelveKey(dungeonName)
    name,                                                -- canonical dungeon name (most recent)
    tiers = { [keyLevel] = { seconds, ... } },           -- rolling window (last 10) per keystone level; 0 = unknown
    xps   = { [keyLevel] = { xpGained, ... } },          -- parallel per-run XP window (nil at max level, i.e. all real M+)
  } },
}?
