...
| `data/worldquests.lua` | Broker `worldquests`: `rewards` — the logged-in (max-level) character's active world-quest **gear** rewards that could upgrade an equipped slot, cached per-character (last-seen; reward data is only readable for the active char). Each entry is a `GearCandidate` + `{questID, title, zone, endTime}`. Scans the current expansion's WQ continent zones (`WQ_CONTINENTS` → `GetMapChildrenInfo`), keeps only equippable gear (`API:ClassifyGearItem`), gated by an ilvl ceiling (no scan when every slot ≥ `WQ_ILVL_CEILING` 220). Throttled on `QUEST_LOG_UPDATE`/`ZONE_CHANGED_NEW_AREA`. `WarbandeerApi:GetWorldQuestRewards`; `/wbc dump wq`. The upgrade *evaluation* lives in ShadowsOfUI-Upgrade (consumes the raw cache) |
...
WarbandeerApi:GetWorldQuestRewards(char?)  → WorldQuestReward[]
    ...
    -- by ShadowsOfUI-Upgrade's WorldQuestUpgrades; empty until a max-level scan
```
...
worldquests = {                                        -- v14; max-level chars only
...
| `worldquests` | rewards (maxLevel) | `QUEST_LOG_UPDATE`, `ZONE_CHANGED_NEW_AREA` (throttled 60s; level-gated in a custom handler) | — |
| `dailies` | (empty) | — | — |
...
- **World-quest scanning is bundled-continent + max-level only.** `data/worldquests.lua`'s `WQ_CONTINENTS` is the current expansion's WQ continent map ID(s) (expanded to zone children at scan time, so all zones are covered regardless of where the player stands); **update it each expansion**. Reward data is only readable for the logged-in character, so alts keep their last-seen cache (expired entries dropped on read by `endTime`). `WQ_ILVL_CEILING` (220) skips the map walk for fully-geared characters. The reward ilvl is the value scaled to the character that scanned it, so it's only ever compared against that same character's slots.
