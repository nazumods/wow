...
| `Model.lua` | `Model` — `ModelScene`-backed 3D viewer (borrows dressup scene 596 for camera + a skinnable player actor); `DisplayInfo(id, useCustomizations?)`/`Unit(token, customRaceID?)`/`TryOn`/`Undress`/`Dress`/`Outfit`/`Scale`. `DisplayInfo` defaults customizations **off** (a baked display carries its own textures; on only matches the player's own race). One player-sized actor renders every model, so large races need `Scale` correction. **Left-drag = actor yaw (rotate), right-drag = camera pan, wheel = camera zoom.** Yaw lives on the actor (so re-skins keep the framing); pan/zoom live on the borrowed scene's `OrbitCamera` (`GetActiveCamera()`): leftX mode set to `NOTHING`, rightX/rightY = `PAN_HORIZONTAL`/`PAN_VERTICAL`, wheel = `ZOOM`, and min/max zoom widened from the scene's tight 6–10 to `minZoom`/`maxZoom` (2/16) so the wheel has travel. Our `OnUpdate`/`OnMouse*` `SetScript`s replace the template's, so they forward to the mixin (`w:OnMouseDown/Up/Wheel`, `w:OnUpdate(elapsed)`) to keep the camera stepping; only `LeftButton` arms the actor-yaw drag. The dragged yaw and remembered `Outfit` (a sourceID list, empty = undressed) are re-applied via a `SetOnModelLoadedCallback` after each async model load (which would otherwise reset the actor to its baked default), so the angle and worn transmog never snap back. `Scale` is instead **re-asserted every frame** in `OnUpdate` — a cold first load resets the actor scale *after* the load-callback/backstop fire, so polling is the only reliable way to keep the per-race size correction (idempotent, corrects within a frame). Use `Outfit` rather than one-shot `TryOn`/`Undress` across a re-skin. A scene actor skins arbitrary races correctly where a bare `DressUpModel` renders them white |
...
| `Model` | inherits Frame (`type = "ModelScene"`, `template = "ModelSceneMixinTemplate"`); `rotateSpeed` (0.01 rad/px), `facing`, `minZoom`/`maxZoom` (2/16, camera zoom range) |
