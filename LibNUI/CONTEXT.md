...
| `Pixels(units)` | A UI-unit length rounded **out** to whole physical pixels (`minPixels = 1`), returned in UI units. Not a pixel count — `Pixels(1)` keeps a border's apparent weight on high-DPI rather than thinning it |
...
- **A 1-unit line is NOT one pixel** (#782). UI units equal physical pixels at exactly one `uiScale`; everywhere else a hairline is a fraction of one (0.97 at the reported scale) and the renderer resolves that to a whole pixel or to nothing, by position. The fraction is close enough to 1 that the phase drifts only ~0.03px per unit, so the misses land on a **fixed stripe every ~34 units** — every widget on one loses the same edge every time, which is why it reads as a broken widget rather than as rounding. Size hairlines with `PixelWidth`/`PixelHeight`/`Inset`, never raw `Width`/`Height`. Sizes are enough; offsets need no snapping of their own. `thickness = 2` was never *invisible* but did render 1px at ~6% of positions, so snapping firms it up rather than changing it. **Verification is scale-dependent** — `/nui test hairlines` shows a raw and a snapped comb next to each other, and a clean pass at one `uiScale` proves nothing about another.
