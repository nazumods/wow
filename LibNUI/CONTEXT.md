...
| `SortableHeaderRow.lua` | `SortableHeaderRow` — standalone clickable column-header strip for custom lists (pairs with VirtualList; TableFrame's sorting stays baked into its grid). `columns` = `{key,label,width,justifyH?,descFirst?}[]`; click selects a column (ascending, or `descFirst`), re-click flips; active column renders accent + arrow texture. `onSort(self,key,desc)` fires on user clicks; `Sort(key?,desc?)` gets/sets silently |
