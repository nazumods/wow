...
| `Model.lua` | `Model` — `ModelScene`-backed 3D viewer (borrows dressup scene 596 for camera + a skinnable player actor); `DisplayInfo(id, useCustomizations?)`/`Unit(token, customRaceID?)`/`TryOn`/`Undress`/`Dress`/`Outfit`/`Scale`. `DisplayInfo` defaults customizations **off** (a baked display carries its own textures; on only matches the player's own race). One player-sized actor renders every model, so large races need `Scale` correction. Built-in drag-to-rotate; the dragged yaw, `Scale`, and the remembered `Outfit` (a sourceID list, empty = undressed) are re-applied via a `SetOnModelLoadedCallback` after each async model load (which would otherwise reset the actor to its baked default), so the angle, size, and worn transmog never snap back. Use `Outfit` rather than one-shot `TryOn`/`Undress` across a re-skin. `Unit` skips the reload and re-dresses in place when the token + `customRaceID` are unchanged (outfit/undress change, or set navigation within one race) — a cached reload doesn't fire the load callback, and reloading an unchanged model flickers; `Outfit` is remember-only and `Unit` performs the single apply. A scene actor skins arbitrary races correctly where a bare `DressUpModel` renders them white |
