...
| `VirtualList` | inherits Frame; `spacing` (2), `padding` (4), `rowHeight` (20), `scrollbar` (true), `items`; single-type `createRow(list)→Frame` + `updateRow(list,row,item,i)→height?`, or multi-type `rowTypes` (`{name={create,update}}`) + `typeOf(item,i)→name` (default `item.type`). Methods: `SetItems(items)`, `Content()`, `Refresh()`, `Row(index)` (live pooled row showing item `index`, for selectable lists), `ScrollToItem(index)` (minimal scroll-into-view) |
