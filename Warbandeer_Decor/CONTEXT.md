...
- **Category dropdown survives the login race.** `BuildFilterStrip` snapshots `CategoryOptions()` once and `FilterDropdown` bakes its menu at construction, so a window opened before the first async scan lands would show only "All decor" forever. `RefreshCategoryFilter()` (called from the standalone `MainWindow:Refresh`, driven by `ns:Scan`) swaps in a freshly-optioned dropdown once the option set grows — preserving the current selection, no-oping when unchanged. The embedded Warbandeer view is built lazily post-scan, so only the standalone window was exposed.
