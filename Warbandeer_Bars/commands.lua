...
-- Temporary diagnostic for L10: shows override map data in a copyable EditBox.
ns:registerCommand("overrides", nil, function()
  ...
  out("== action bar: slot spellId -> GetOverrideSpell ==")
  n = 0
  for i = 1, 72 do
    local t, index = GetActionInfo(i)
    if t == "spell" and index then
      local ovr = C_Spell.GetOverrideSpell(index)
      out(("  s%d:%d(%s)->%d(%s)"):format(i, index, name(index), ovr, name(ovr)))
      n = n + 1; if n >= 6 then break end
    end
  end
  if n == 0 then out("  (none)") end

  out("== first flyout on bar: GetFlyoutSlotInfo(id,1) ==")
  local found = false
  for i = 1, 72 do
  ...
  eb:SetPoint("TOPLEFT",     f, "TOPLEFT",     8, -8)
  eb:SetPoint("BOTTOMRIGHT", f, "BOTTOMRIGHT", -8, 36)
  ...
  hint:SetPoint("BOTTOMLEFT", f, "BOTTOMLEFT", 8, 12)
  hint:SetText("Ctrl+A  Ctrl+C to copy — drag to move — click elsewhere to close")

  f:SetScript("OnMouseDown", function(_, btn)
    if btn == "RightButton" then f:Hide() end
  end)
  f:Show()
  ns.Print("Spell override dump open — Ctrl+A, Ctrl+C, then paste here.")
end, "Show spell override maps in a copyable window (L10 diagnostic)")
