...
| `src/currencies.rs` | Builds the **Currencies tab**'s grid (#884): a row per character, a column per currency the addon's broker persists, joined to bundled names/icons through `static-data.json`'s `currencyFields` (a save keys currencies by the broker's FIELD NAME and carries no id). `CURRENCY_ORDER` is the app's own column order, mirroring the in-game `.toc` load order with gold last; a mapped field it doesn't list is **appended, not dropped**. Returned on the `get_overview` payload rather than a command of its own — same parsed save, so a second command would mean a second mlua exec |
