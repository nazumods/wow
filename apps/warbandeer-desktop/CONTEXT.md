...
| `src/model.rs` | Typed **subset** of `WarbandeerCharDB` (serde, unknown fields ignored): warband gold/week, per-char basic/spec/professions/equipment/currency/playtime/reputations/guid. `Currency` is the exception to "typed subset": it's the raw field map (`#[serde(transparent)]` over `HashMap<String, CurrencyValue>`) so a broker field added later isn't dropped, with `gold()`/`get()` accessors. `CurrencyValue` is an untagged `Scalar \| Table` — see Gotchas |
...
| `src/currencies.rs` | Builds the **Currencies tab**'s grid (#884): a row per character, a column per currency the addon's broker persists, joined to bundled names/icons through `static-data.json`'s `currencyFields` (a save keys currencies by the broker's FIELD NAME and carries no id). `CURRENCY_ORDER` is the app's own column order, mirroring the in-game `.toc` load order with gold last; a mapped field it doesn't list is **appended, not dropped**. Each column carries its own `icon` and `format` (`money`/`count`) the way each in-game column object carries `iconPath` + `getData` — there is no gold flag, and `standin_icon()` assigns app-owned `wb:*` icon names by **currency id** (stable, lint-gated) rather than by DB2's regenerated-weekly art string. Returned on the `get_overview` payload rather than a command of its own — same parsed save, so a second command would mean a second mlua exec |
...
| `src/staticdata.rs` | Offline lookup layer — `data/static-data.json` embedded via `include_str!`, parsed once into a `OnceLock`. `currency(id)` resolves a currency id to name / icon name / cap / quality; `currency_field(name)` resolves a broker field name to its currency id (the `currencyFields` map, #884); `achievement(id)` resolves a **tracked** achievement id to name / icon name / points. Generated data, never hand-edited |
...
| `lib/components/Currencies.svelte` | The Currencies tab's grid (#884). Icon-over-name headers (`<Icon>` + the currency's DB2 name — no hand-maintained labels), sticky character column, quantity right-aligned. Mirrors `views/SummaryColumns.lua`: capped cells `var(--red)` (`ns.CappedColor`), a captured 0 a muted em-dash (`ns.ZeroDash`). The dash is gated on the **data**, not on max level as in game — the broker never captures its `maxLevel = true` fields for a levelling character, so a null cell already IS that case and renders blank |
...
| `lib/components/Achievements.svelte` | Per-expansion checklist from `db.achievements` joined to the bundled catalog (#731). Completed rows `var(--green)`, the rest `var(--muted)` — muted rather than the in-game DIM_RED, since an unearned achievement is a to-do, not an error. **With icons (#883):** each row renders its bundled icon NAME as an image through `Icon.svelte` — the `wbicon` scheme served out of the embedded `icons.bin`, with a same-size placeholder when a name has no image — the same shared icon capability the Currencies tab uses. `wasEarnedByMe` is deliberately not shown — the addon documents it as last-captured-character-wins, not per-alt. Labels for the catalog keys live here; ORDER comes from the backend's `EXPANSION_ORDER`, mirroring `Warbandeer/views/Overview.lua`'s `EXPANSIONS` |
...
| `../../Warbandeer_Characters/data/currency.lua` | Not ours either, but `currencyFields` is parsed from it — its inline `id` per broker field is the **only** place the field-name ↔ currency-id mapping exists. Adding a field there needs a bundle regeneration or the column never appears; `Tooling/lint-stale-ids.ps1`'s `currency.id` rule gates a stale id at PR time |
...
|---|---|---|
| `get_overview` | `Overview` | Picks the account whose SavedVariables file is most recently modified. Carries the **Currencies tab's** grid too (`overview.currencies`), not just the Overview view's data — one parse of the save serves both |
...
| `get_achievement_meta(id)` | `AchievementMeta \| null` | Name / icon name / points from the embedded bundle. Covers only tracked ids, so `null` means "not tracked", not "no such achievement" |
| `static_data_build` | `string` | Client build the embedded bundle came from — surfaces a stale bundle in diagnostics |

Operator-only ops commands — supplied by the shared [`apps/bot-ops`](../bot-ops/README.md) module and
re-exported through `lib/api.ts`, not defined in this app (no `wowDir`; each takes the selected
`target` index and shells `ssh <ssh> "BOT_OPS_PROJECT=<p> BOT_OPS_CONTAINER=<c> bash <remoteDir>/ops/bot-ops.sh …"`,
see `../warbandeer-discord/ops/README.md`):
...
- **A static-data refresh cuts a release, but no longer needs a hand-written version bump.** `data/static-data.json` is embedded with `include_str!`, so a data change really does need a new exe — and `app-release.sh`'s doc-only exclusion is `**/*.md`, so the bundle counts as a real app change. Since #700 the daily `app-release.yml` cron bumps the version itself, so merging a refresh no longer fails the release build the way it did when #660 landed. `update-static-data.yml` stays weekly and PR-only so a data diff still gets a human glance.
- **A release tag that doesn't start with `app-` triggers the CurseForge publisher.** `publish.yml` runs on *every* `release: published` and skips only `app-*` tags, so the static-data release tag is `app-static-data-v…` deliberately — it reuses the existing guard instead of adding a second rule. Renaming it without touching `publish.yml` would push a data blob at CurseForge.
...
- **The bundle holds what the addon *doesn't* persist, not everything DB2 offers.** #639 proposed nine tables; #631–#637 then shipped as addon-side persistence, settling the open "does this app gain Blizzard credentials?" question as **no** — so the title catalog, keystone/mastery names, pet species and reputation thresholds all come from SavedVariables and are deliberately *not* bundled. Only currencies (no REST equivalent exists) and achievements (`data/achievements.lua` persists completion bits only) qualify. Adding a table means clearing that same bar, not just finding it in DB2.
- **A save keys currencies by NAME, the bundle by ID, and nothing in SavedVariables links them.** `toon.currency` uses the broker's hand-written field names (`HeroDawncrest`); `static-data.json` uses currency ids (`"3345"`). The map exists only inline in `Warbandeer_Characters/data/currency.lua` and is bundled as `currencyFields` (#884) — so **the join is only as fresh as the bundle**. Hardcoding the list here instead was rejected: it duplicates a hand-maintained table across a repo boundary and drifts silently the next time a field is added.
- **A currency entry has five shapes, one of which is a bare number.** The broker writes four scalar fields and four different table shapes (`{quantity,max,capped}`, `+earned`, `+weeklyMax`, and all of them for `ShardOfDundun`) — hence `CurrencyValue` being an untagged enum over one all-defaulted `CurrencyTable`. The trap is `NebulousVoidcore`: real saves can still hold it as a **plain number** from before it became a table (`currency.lua`'s own `reset` carries a `type(c) == "number"` branch), so a reader that assumes a table fails on live data.
- **`quantity: null` is a render case, not missing data.** `GetCurrencyInfo` returns nil for a currency the character has never discovered, and the broker skips its `maxLevel = true` fields entirely for a levelling character — so an absent cell renders blank while a captured `0` renders the em-dash. Collapsing the two loses the distinction the in-game view spends a max-level check to make.
- **Ten of the eleven currency columns carry an icon; `Untainted Mana-Crystals` does not.** Its DB2 icon name has a literal space (`jewelcrafting_uncut epic gem_color1`), one of the 9 names the CDN carries under no spelling, so it packs empty and renders the placeholder. Gold has no icon for a different reason — it is `GetMoney()` copper, not a currency row, so there is nothing to look one up from. Both are why the headers name every column instead of being icon-only.
- **An achievement lookup missing is not an error.** The extract is filtered to `achievementcatalog.lua`, so `get_achievement_meta` returns `null` for any real achievement Warbandeer doesn't track. Callers must render that case rather than treating it as a stale bundle.
...
- **The Ops tab's backend is not this app's to change alone.** Everything below `BotOps.svelte` lives in [`apps/bot-ops`](../bot-ops/README.md) and is vendored into `roshne/wow-companion`, so a change here reaches a second shipped app. Edit the module, run its `cargo test`, and re-vendor downstream — never patch the behaviour inside this app to work around it. Two knock-ons worth knowing: `apps/bot-ops` needs its own `npm install` (that is what lets the out-of-tree TS resolve `@tauri-apps/api`), and `vite.config.ts` dedupes that package so it is not bundled twice.
