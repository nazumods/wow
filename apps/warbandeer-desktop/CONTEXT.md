...
| `lib/components/Achievements.svelte` | Per-expansion checklist from `db.achievements` joined to the bundled catalog (#731). Completed rows `var(--green)`, the rest `var(--muted)` — muted rather than the in-game DIM_RED, since an unearned achievement is a to-do, not an error. **Text only:** the bundle carries a bare icon NAME and this app resolves no game icons at all yet (currencies have the same unused names), so that is split out as one shared capability. `wasEarnedByMe` is deliberately not shown — the addon documents it as last-captured-character-wins, not per-alt. Labels for the catalog keys live here; ORDER comes from the backend's `EXPANSION_ORDER`, mirroring `Warbandeer/views/Overview.lua`'s `EXPANSIONS` |
