...
| `src/commands.ts` | `/dmf`, `/reset`, `/status` handlers; command names carry the optional `config.commandPrefix` (stripped back off on dispatch); Discord `<t:…>` timestamp helpers |
