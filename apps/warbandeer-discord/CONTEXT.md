...
| `ops/bot-ops.sh` + `ops/README.md` | Operator admin surface for the desktop app's **Ops** tab (`apps/warbandeer-desktop`): whitelisted `status`/`logs`/`restart`/`env-get`/`env-set` over docker+`.env`, invoked over SSH. Secrets are never read/written (whitelist excludes them); env-set backs up then `-p warbandeer-discord-debug up -d --force-recreate`. Backups go to `BOT_OPS_BACKUP_DIR` (default `${XDG_STATE_HOME:-$HOME/.local/state}/warbandeer-bot-ops`, `0700`, named `env-<project>.bak.<stamp>`) — **never beside `.env`**, since that dir is a public-repo checkout and a backup there is an untracked file holding the live token. The only privileged surface — the desktop app just SSHes to it |
