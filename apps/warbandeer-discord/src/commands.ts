    ...
    case "update": {
      ...
      await withCritical(async () => {
        try {
          ...
          await interaction.editReply(updateReply(decision, latestSha, reason));
        } catch (err) {
...
export function updateReply(
  ...
  switch (decision) {
    case "disabled":
      ...
      if (reason === "unpublished-sha") {
        return (
          `⚠️ Self-update is disabled — this build's commit \`${config.gitSha?.slice(0, 7)}\` ` +
